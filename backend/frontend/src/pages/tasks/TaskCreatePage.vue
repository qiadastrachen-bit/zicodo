  ...
  <div class="schedule-page">
    ...
    <div v-else class="team-content">
      ...
      <template v-if="teamsStore.currentView === 'list'">
        ...
        />
        <EmptyState v-if="!teamsStore.teams.length" message="您还没有加入团队" description="创建团队或输入邀请码加入" />
        <div v-if="!teamsStore.teams.length" class="empty-actions">
          <button class="join-team-btn" @click="showJoinForm = true">
            <ZlIcon name="LogIn" :size="18" />
            <span>输入邀请码加入</span>
          </button>
          <button class="create-team-btn" @click="showCreateForm = true">
