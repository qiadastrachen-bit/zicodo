 ...
 * ─────────────────────────────────────────────────────────
 * 支持：
 *   1. 通义千问（DashScope）—— 配置 DASHSCOPE_API_KEY
 *   2. 文心一言  —— 配置 ERNIE_API_KEY + ERNIE_SECRET_KEY
 *   3. Mock 模式 —— 未配置 Key 时自动 fallback，答辩演示可用
 *
 * POST /api/ai/chat
 *   body: { message: string }
...
/**
 * 通义千问 API（DashScope）流式调用
 */
async function callTongyiStream(systemPrompt, userMessage, onChunk, onDone) {
...
async function chat(req, res) {
  ...
  try {
    if (process.env.DASHSCOPE_API_KEY) {
      await callTongyiStream(systemPrompt, message, sendChunk, sendDone);
    } else {
      // fallback：mock 模式
      console.warn('[AI] 未配置 API Key，使用 Mock 模式');
